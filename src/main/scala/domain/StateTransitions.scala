...
import monads.State
import monads.State.given

object StateTransitions:

  def receiveMaterial(mats: Map[Material, Int]): State[StateData, Unit] =
    State.modify { s =>
      s.copy(materials = combineMaps(s.materials, mats))
    }

  // Собрать партию игрушек (требует конфигурации)
  def runAssembly(
    config: Config,
    state: StateData,
    product: Product,
    quantity: Int
  ): (Vector[String], StateData, Either[String, (Int, Int)]) =
    val recipe = config.recipes(product)
    val enough = recipe.materials.forall { case (m, need) =>
      state.materials.getOrElse(m, 0) >= need * quantity
    }
    if (!enough) {
      (Vector(s"Не хватает материалов для $product в количестве $quantity"), state, Left("Недостаточно материалов"))
    } else {
